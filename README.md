...
- `copado-apps-script-webapp` `0.1.3`: private-by-default static, read-only Sheet, and vendored Canvas Apps Script web apps with stable `/exec` deployment safeguards.
...
Pin an independent skill release with `--version 0.1.3`. Existing installations are not overwritten unless `--force` is explicit.
...
Packages use namespaced versions such as `copado-apps-script-webapp-v0.1.3`; releasing one skill does not imply other skills changed.
