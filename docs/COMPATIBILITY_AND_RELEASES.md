...
- Skill `0.1.6` pins template `v0.1.4`.
- Cursor and Claude Code install the same `skills/copado-apps-script-webapp` payload.
- Node.js 20 or newer is required for validation, initialization, and browser smoke tests.
...
The template tag is an immutable executable dependency. Never change the pin to `main`. Executable changes release in the template first; the skill then updates `template.json`, runs all profile fixtures, and releases independently.

## Upgrade procedure

1. Read both the template and skill release notes.
2. Review the exact new template tag and its manifest/workflow changes.
3. Update `template.json` in a skill branch.
...
Until those gates pass, the current compatible pair remains skill `0.1.6` with template `v0.1.4`. Do not label either repository `v1.0.0` based only on local installation into a Claude skill directory.
